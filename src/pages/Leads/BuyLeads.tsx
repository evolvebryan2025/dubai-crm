...
const BuyLeads: React.FC = () => {
  ...
  // Data state
  const [buyLeads, setBuyLeads] = useState<Lead[]>([]);
    ...
    const fetchData = async () => {
      ...
      try {
        const [leadsRes, teamsRes, profilesRes] = await Promise.all([
          leadsService.getBuyLeads(),
          teamsService.getAll(),
          profilesService.getAll(),
        ]);

        if (leadsRes.data) {
          setBuyLeads(leadsRes.data.map((l: any) => supabaseLeadToLead(l)));
        }
        if (teamsRes.data) {
        ...
        console.error('Error fetching buy leads data:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const filteredBySearch = useMemo(() => {
    if (!searchText) return buyLeads;
    const lower = searchText.toLowerCase();
    return buyLeads.filter(
      (lead) =>
  ...
  // Initialize kanban leads when buyLeads loads
  useEffect(() => {
    setKanbanLeads(buyLeads);
  }, [buyLeads]);
  ...
  const renderKanbanView = () => {
    const kanbanBuyLeads = kanbanLeads.filter((l) => l.lead_type === 'Buy');
    return (
      <div
        style={{
          display: 'flex',
      ...
      >
        {kanbanColumns.map((col) => {
          const colLeads = kanbanBuyLeads.filter((l) => l.status === col.status);
          return (
            <div
              key={col.status}
  ...
  return (
    <div style={{ padding: 24 }}>
      {/* Page title */}
      <h2 style={{ margin: 0, marginBottom: 20, fontWeight: 600, fontSize: 22 }}>Buy Leads</h2>

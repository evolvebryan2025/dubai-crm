...
import type { RentListing, User, Team } from '../../types';
import {
  listingsService,
  teamsService,
  profilesService,
...
function mapStatusToUI(status: string): 'Active' | 'Inactive' | 'Rented' {
  switch (status) {
    case 'active':
    case 'under_offer':
      return 'Active';
...
function getRentFrequencyDisplay(raw: RawListing): string {
  if (raw.rent_frequency) {
    // Capitalize first letter
    return raw.rent_frequency.charAt(0).toUpperCase() + raw.rent_frequency.slice(1);
  }
  return 'Yearly';
}
...
const RentList: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [listingsRes, teamsRes, profilesRes, developersRes] =
        await Promise.all([
          listingsService.getRentListings(),
          teamsService.getAll(),
      ...
      if (listingsRes.data) {
        ...
        setRentListings(
          listingsRes.data.map((l: any) => listingToRentListing(l))
        );
      }
      if (teamsRes.data) {
      ...
      console.error('Error fetching rent listings data:', error);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const tableRows: TableRow[] = useMemo(() => {
    return rawListings.map((raw) => {
      ...
      return {
        ...
        _rentFrequencyDisplay: getRentFrequencyDisplay(raw),
      };
    });
  }, [rawListings, developerMap]);

  // ------------------------------------------
  // Tab counts (from RentListing adapted data)
  // ------------------------------------------
  const tabCounts = useMemo(() => {
    const active = rentListings.filter((l) => l.status === 'Active').length;
    const inactive = rentListings.filter((l) => l.status === 'Inactive').length;
    const rented = rentListings.filter((l) => l.status === 'Rented').length;
    const my = rentListings.filter((l) => l.agent_id === '1').length;
    return { active, inactive, rented, my };
  }, [rentListings]);

  // Unique tags from adapted listings
  const allTags = useMemo(() => {
    const tagSet = new Set<string>();
    rentListings.forEach((l) => l.tags.forEach((t) => tagSet.add(t)));
    return Array.from(tagSet);
  }, [rentListings]);

  // ------------------------------------------
  // Filtering (works on both rentListings and tableRows)
  // ------------------------------------------
  const filteredRentListings = useMemo(() => {
    let result = [...rentListings];
  ...
  const filteredIdSet = useMemo(() => {
    return new Set(filteredRentListings.map((l) => l.id));
  ...
  const columns: ColumnsType<TableRow> = [
    ...
    {
      title: 'Rental Price (AED)',
      dataIndex: 'price',
      key: 'price',
      width: 170,
      render: (value: number) => (
        <Text strong>{formatAED(value)}</Text>
      ),
    ...
    },
    {
      ...
      width: 110,
      render: (_: unknown, record: TableRow) => (
        <Tag color="purple">{record._rentFrequencyDisplay}</Tag>
      ),
    },
    {
      title: 'Beds',
          ...
          <Col xs={24} sm={12} lg={8} xl={6} key={listing.id}>
            ...
            >
              <Title level={5} style={{ margin: 0, color: PRIMARY_COLOR }}>
                {formatAED(listing.rental_price)}
              </Title>

              <div style={{ margin: '4px 0', fontSize: 12, color: '#8c8c8c' }}>
                {listing.listing_id}
              </div>

              <Text type="secondary" style={{ fontSize: 12 }}>
                {listing.rental_frequency}{' '}
                {listing.cheques ? `| ${listing.cheques} Cheques` : ''}
              </Text>

              <Space size={16} style={{ margin: '8px 0', display: 'flex' }}>
                <Space size={4}>
  ...
  const tabItems = [
    ...
    { key: 'Rented', label: `Rented (${tabCounts.rented})` },
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      {/* Content */}
      <Badge.Ribbon
        text={`${filteredRentListings.length} listings`}
        color={PRIMARY_COLOR}
        style={{ display: 'none' }}
      >
        <div>
